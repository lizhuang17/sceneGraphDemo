import {
  ...
  Vector3,
  WebGLRenderer,
  PlaneGeometry,Mesh,MeshBasicMaterial,DoubleSide
...
import {People} from "./old_people.js"
...
export class Viewer
{
  constructor (el, options)
  {
    ...
    // var mat = new Matrix4()
    // mat.copy(instanceGroup.matrixWorld).inverse()

    var speed = 10
    this.vc = new Vector3(-10,0,0)
    ...
    var groundGeom = new PlaneGeometry(100000, 100000, 4, 4);
    this.groundMesh = new Mesh(groundGeom, new MeshBasicMaterial({ color: 0xffffff , side: DoubleSide}))
    scene.add(this.groundMesh)
    this.groundMesh.visible=false
    this.onKeyDown = function (event){
      if(event.key==="l"||event.key==="L"){
        _self.people = new People()
        _self.people.load(window.scene, window.camera);
      }
      else if(event.key==="F"||event.key==="f"){
        const geometry = new BufferGeometry();
				const positions=[0,0,100]
				const size = [500+200*Math.random()]
				geometry.setAttribute( 'position', new Float32BufferAttribute( positions, 3 ) );
				geometry.setAttribute( 'particleSize', new Float32BufferAttribute( size, 1 ) );
        //const targetPosition = attribute( 'position', 'vec3' );
				const time = timerLocal();
				const fireMap = new TextureLoader().load( 'firetorch_1.jpg' );
				const fireUV = spritesheetUV(
					vec2( 6, 6 ), // count
					pointUV, // uv
					time.mul( 100 ) // current frame
				);
				const fireSprite = texture( fireMap, fireUV );
				const fire = fireSprite.mul( 10 );//控制火焰大小
        ...
        //const lerpPosition = uniform( 0 );
				...
				//material.sizeNode = particsleSize;
        //material.positionNode = positionNode;
				const particles = new Points( geometry, material );
				_self.sceneEx.add( particles );
      }
      ...
      console.log(_self.groundMesh.position)
    }
  ...
  setCamera()
  {
      ...
      if(window.projectName==="HaiNing"){
        ...
        var cen = [163.7,107.0,102.8] 
      }else if(window.projectName==="KaiLiNan"){
        var pos = [-440.7,40.0,103.4]
        var cen = [-459.7,26.6,73.9]
      }else if(window.projectName==="LanQiao"){
        var pos = [181.1,512.9,913.8]
        var cen = [171.8,96.6,242.5]
      }else if(window.projectName==="QinLaiLi"){
        var pos = [-182.4,251.1,-306.4]
        var cen = [-103.7,40.9,-7.8]
      }else if(window.projectName==="RenFuYiYuan"){
        var pos = [320.0,5169.8,-7571.8]
        var cen = [320.0,435.9,-2944.2]
      }else if(window.projectName==="XinYu"){
        var pos = [224.4,138.6,-221.4]
        var cen = [257.9,-13.9,-88.5]
      }else if(window.projectName==="YunXi"){
        var pos = [103.2,389.4,287.9]
        var cen = [38.8,43.0,-30.6]
      }
      var vp = this.update_matrix(pos)
      //var vc = this.update_matrix(cen)
  ...
  update_matrix(pos){
    ...
    // scale =1
    // console.log(m[12]/scale*tran[0],m[13]*tran[1],m[14]/scale*tran[2])
    ...
    //this.defaultCamera.applyMatrix(new_m)
  ...
  addMyUI()
    {
      ...
      for(var i=0;i<config.length;i++){
        ...
        // var camera_pos0=this.update_matrix(camera_pos[ nums[i] ])
        // var camera_rot0=this.update_matrix(camera_rot[ nums[i] ])
        // console.log(nums[i])
